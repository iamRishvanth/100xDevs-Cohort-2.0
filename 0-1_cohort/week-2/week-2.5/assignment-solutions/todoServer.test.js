  ...
  test('should retrieve all todo items', (done) => {
    http.get(`${baseUrl}/todos`, (res) => {
      ...
      res.on('end', () => {
        ...
        expect(Object.prototype.toString.call(todos) === '[object Object]').toBe(true);
        expect(Object.keys(todos).length).toBe(1);
        expect(todos[createdTodoId].title).toBe(todo.title);
        expect(todos[createdTodoId].description).toBe(todo.description);
        done();
      });
    });
  });
